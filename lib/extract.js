...
async function performExtract(client, opts = {}) {
  ...
  const brief = { ...base, elements, text, regions, visualImages, lookup, stats };
  try {
    const { buildScreenGraph } = require('./graph');
    brief.screenGraph = buildScreenGraph(brief);
  } catch (err) {
    // Non-fatal fallback if graph builder fails
  }
  return brief;
}
