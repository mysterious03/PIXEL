...
// Run the Step-0 planning call with Local SLM tiering and Confidence Routing.
// If models.primaryLocal is configured, tries the local SLM first, evaluates confidence,
// and only escalates to cloud if confidence is below threshold or local fails.
async function generatePlan({ task, context, provider, model, cacheKey, reasoningEffort, maxTokens, timeoutMs, config, auditTrail }) {
  const cfg = config || {};
  const localCfg = cfg.models?.primaryLocal;
  const router = require('./router');

  // Check if Local SLM tier is enabled & available
  if (localCfg && localCfg.model) {
    try {
      const message = buildPlanMessage({ task, context });
      const localProvider = localCfg.provider || 'ollama';
      const localModel = localCfg.model || 'qwen2.5:1.5b-instruct';
      const localTimeout = localCfg.timeoutMs || 10000;

      const localCompletion = await callModel(
        { system: PLAN_SYSTEM, tools: [], messages: [message], model: localModel, cacheKey, maxTokens: 512, timeoutMs: localTimeout },
        { provider: localProvider }
      );

      const localText = (localCompletion.text || localCompletion.refusal || '').trim();
      
      // Calculate plan confidence: check if plan is non-empty, actionable, and length is reasonable
      const isActionable = localText.length >= 30 && !localCompletion.refusal;
      const confidence = isActionable ? 0.90 : 0.40;
      const decision = router.routeDecision(confidence);

      if (decision.route === 'LOCAL' || decision.route === 'LOCAL_CONFIRM') {
        if (auditTrail && typeof auditTrail.logTurn === 'function') {
          auditTrail.logTurn({
            role: 'reasoning',
            routeDecision: 'LOCAL',
            modelTierUsed: `Local SLM (${localModel})`,
            offDevicePayloadSent: false,
            action: { type: 'step0-plan', planLength: localText.length },
          });
        }
        return { text: localText, completion: localCompletion, routedTier: 'LOCAL' };
      }

      // If confidence < tauLow, record escalation reason and escalate to cloud
      if (auditTrail && typeof auditTrail.logTurn === 'function') {
        auditTrail.logTurn({
          role: 'reasoning',
          routeDecision: 'CLOUD',
          modelTierUsed: `Cloud (${model || 'cloud-plan'})`,
          escalationReason: 'Local SLM plan confidence below threshold',
          offDevicePayloadSent: true,
          action: { type: 'step0-plan' },
        });
      }
    } catch (err) {
      // Local SLM offline or failed -> fallback to cloud
      if (auditTrail && typeof auditTrail.logTurn === 'function') {
        auditTrail.logTurn({
          role: 'reasoning',
          routeDecision: 'CLOUD',
          modelTierUsed: `Cloud (${model || 'cloud-plan'})`,
          escalationReason: `Local SLM unavailable: ${err.message}`,
          offDevicePayloadSent: true,
          action: { type: 'step0-plan' },
        });
      }
  ...
  // Cloud model call (fallback or direct)
  const message = buildPlanMessage({ task, context });
  ...
  return { text, completion, routedTier: 'CLOUD' };
}
