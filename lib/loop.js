...
async function run({ session, task, config = {} } = {}) {
  ...
  async function notifyHud(status, message, color = null) {
    if (!session?.client?.Runtime?.evaluate) return;
    try {
      const { generateUpdateHudStatusScript, generateLiveHudScript } = require('./live-hud');
      // Ensure HUD exists in the DOM; if page navigated or reloaded, re-inject immediately
      const checkRes = await session.client.Runtime.evaluate({
        expression: `Boolean(document.getElementById('__pixel_live_hud_root__'))`,
        returnByValue: true,
      }).catch(() => null);
      if (!checkRes?.result?.value) {
        await session.client.Runtime.evaluate({ expression: generateLiveHudScript() }).catch(() => {});
      }
      await session.client.Runtime.evaluate({
        expression: generateUpdateHudStatusScript({
          status,
          message,
          color: color || (status === 'THINKING' ? '#FF9F0A' : (status === 'ACTING' || status === 'EXECUTING') ? '#BF5AF2' : status === 'NAVIGATING' ? '#0A84FF' : status === 'FAILED' ? '#FF453A' : '#30D158')
        }),
      }).catch(() => {});
    } catch (_) {}
  }
  ...
  try {
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      // Universal Overlay & Cookie Banner Auto-Dismissal:
      // Real websites (Google, Amazon, Instagram, etc.) frequently show consent modals.
      // Automatically dismiss them so the agent sees and interacts with the real page.
      if (session.client?.Runtime?.evaluate) {
        try {
          await session.client.Runtime.evaluate({
            expression: `(() => {
              ...
              for (const btn of buttons) {
                ...
                if (dismissTerms.test(text) && btn.offsetParent !== null) {
                  btn.click();
                  return true;
                }
              }
      ...
      // Automatic On-Device Privacy Shield: Automatically blur all sensitive PII
      // (passwords, emails, phone numbers, cards, SSN, delivery addresses, etc.)
      // on the live browser page on every turn without requiring user commands.
      if (session.client?.Runtime?.evaluate) {
        try {
      ...
      if (prevUrl !== null && brief.url && brief.url !== prevUrl) {
        ...
        await notifyHud('NAVIGATING', `Navigated to ${cleanUrl(brief.url, 40)}`, '#0A84FF');
