...
// ODVPA Audit & Metrics — wired into every perception cycle
const { LocalAuditTrail } = require('./audit');
const { EscalationTracker } = require('./router');
const { calculatePrivacyRisk } = require('./privacy');
const { buildScreenGraph, minimiseScreenGraph } = require('./graph');
const { diffScreenGraphs } = require('./diff');
...
async function run({ session, task, config = {} } = {}) {
  ...
  // ODVPA: Per-run audit trail + escalation tracker
  // These are instantiated fresh for every run so metrics never bleed across runs.
  const auditTrail = new LocalAuditTrail();
  const escalationTracker = new EscalationTracker();
  let visionCallsThisRun = 0;   // incremented every time take_screenshot executes
  let structuralStepsThisRun = 0; // turns resolved structurally via AXTree/DOM
  let visualStepsThisRun = 0;     // turns requiring visual perception
  let totalPixelsSaved = 0;       // cumulative pixels saved by cropping
  let prevScreenGraph = null;   // previous turn's Screen Graph for dirty-region diff
  ...
  // Real-time on-screen Dynamic Island status & telemetry.
  // Single CDP round-trip per call: fires a self-healing update script that
  // re-injects the HUD only when it is missing, then sets the status badge.
  // Fire-and-forget (returns the promise without awaiting) so callers don't block.
  let _hudLastKnownMissing = true; // assume missing on first turn; set false once confirmed present
  function notifyHud(status, message, color = null) {
    if (!session?.client?.Runtime?.evaluate) return Promise.resolve();
    const { generateUpdateHudStatusScript, generateLiveHudScript } = require('./live-hud');
    const resolvedColor = color || (status === 'THINKING' ? '#FF9F0A' : (status === 'ACTING' || status === 'EXECUTING') ? '#BF5AF2' : status === 'NAVIGATING' ? '#0A84FF' : status === 'FAILED' ? '#FF453A' : '#30D158');
    const updateScript = generateUpdateHudStatusScript({ status, message, color: resolvedColor });
    // If HUD is known to be present, skip the existence check (no extra CDP hop)
    if (!_hudLastKnownMissing) {
      return session.client.Runtime.evaluate({ expression: updateScript }).then(r => {
        // If the update had no effect (badge is null/undefined), HUD must be gone
        if (!r?.result) _hudLastKnownMissing = true;
      }).catch(() => { _hudLastKnownMissing = true; });
    }
    // HUD is missing or unknown: inject first, then update in two sequential evals
    // but use Promise chaining so the outer call is still non-blocking
    return session.client.Runtime.evaluate({ expression: generateLiveHudScript() })
      .then(() => session.client.Runtime.evaluate({ expression: updateScript }))
      .then(() => { _hudLastKnownMissing = false; })
      .catch(() => {});
  }
  ...
  try {
    ...
    await notifyHud('THINKING', `Planning task: "${task.length > 40 ? task.slice(0, 39) + '…' : task}"...`);
    ...
    while (iter < loopCfg.maxSteps) {
      ...
      if (++totalIterations > maxIterations) {
        ...
        return await finish(runArtifact, { scratchpad, config: cfg, auditTrail, escalationTracker, visionCallsThisRun });
      }
      if (exec.backend.waitUntilReady) await exec.backend.waitUntilReady();
      await notifyHud('THINKING', `Turn ${iter}/${loopCfg.maxSteps} · Perceiving page layout...`);
      ...
      // Turn 1 Zero-Cost Direct Navigation:
      // If on a blank/newtab page, or if the user's task explicitly directs to another
      // website/service, navigate directly without wasting an LLM turn.
      if (iter === 1) {
        try {
          const destUrl = resolveDestinationUrl(task, brief.url);
          const isBlank = !brief.url || brief.url === 'about:blank' || brief.url.startsWith('chrome://');
          if (destUrl && (isBlank || (destUrl !== brief.url && !brief.url.includes(new URL(destUrl).hostname)))) {
            emit(`  ↳ Navigating directly to destination: ${cleanUrl(destUrl, LOG_URL_MAX)}`);
            try {
              const host = new URL(destUrl).hostname;
              await notifyHud('NAVIGATING', `Navigating to ${host}...`, '#38bdf8');
            } catch (_) {}
            await session.client.Page.navigate({ url: destUrl });
            await waitUntilLoaded(session.client);
            brief = await session.extract({ inViewportOnly: true });
            prevUrl = destUrl;
          }
        } catch (_) {}
      }

      // Universal Overlay & Cookie Banner Auto-Dismissal + Privacy Shield:
      // Throttled to every 3 turns (fire-and-forget) — these CDP calls add latency
      // on every turn without benefit after the banner is dismissed / fields are blurred.
      if (iter % 3 === 1 && session.client?.Runtime?.evaluate) {
        const { generatePrivacyBlurScript } = require('./live-hud');
        // Fire both CDP calls concurrently and don't await — they're best-effort
        session.client.Runtime.evaluate({
          expression: `(() => {
              const dismissTerms = /^(accept all|accept cookies|allow all|i agree|agree|accept|dismiss|not now|close|got it|continue)$/i;
              const btns = Array.from(document.querySelectorAll('button, a, [role="button"], input[type="button"]'));
              for (const b of btns) { const t = (b.textContent || b.value || '').trim(); if (dismissTerms.test(t) && b.offsetParent) { b.click(); return true; } }
              return false;
            })()`,
        }).catch(() => {});
        session.client.Runtime.evaluate({
      ...
      if (prevUrl !== null && brief.url && brief.url !== prevUrl) {
        ...
        _hudLastKnownMissing = true; // new document — always re-inject HUD
        notifyHud('NAVIGATING', `Navigated to ${cleanUrl(brief.url, 40)}`, '#0A84FF');
      ...
      // 2. Reduce (Task-Aware Context Minimisation: prunes irrelevant nodes)
      const llmView = reduce(brief, cfg.view, task);
      ...
      // 2.5 Zero-Cost Deterministic Fast-Path (0 Tokens / 0 API Calls)
      let completion = null;
      if (iter === 1 && events.length === 0) {
        const currentUrl = brief.url || '';
        const targetUrl = resolveDestinationUrl(task, currentUrl);
        if (targetUrl) {
          try {
            const targetHost = new URL(targetUrl).hostname;
            if (!currentUrl.includes(targetHost)) {
              completion = {
                provider: 'on_device_fastpath',
                model: 'deterministic-heuristic',
                actions: [{ verb: 'navigate', args: { url: targetUrl, intent: `Navigate directly to ${targetUrl}` } }],
                usage: { inputTokens: 0, outputTokens: 0 },
                text: `Directly navigating to ${targetUrl}`
              };
            }
          } catch (e) {}
        }
      ...
      if (!completion) {
        await notifyHud('THINKING', `Turn ${iter}: Analyzing page & formulating plan...`, '#FF9F0A');
      ...
      let plannedActions = Array.isArray(completion.actions) ? [...completion.actions] : [];
      const modelText = (completion.refusal || completion.text || '').trim();

      // Autonomous Recovery for Local SLMs / Conversational Model Outputs:
      // When local models output natural language explanations instead of calling tools,
      // resolve the intended action against the live page elements.
      if (plannedActions.length === 0 && modelText) {
        const lowerText = modelText.toLowerCase();

        // 1. Check if modelText mentions clicking any specific element visible on the page
        if (brief?.elements?.length) {
          for (const el of brief.elements) {
            if (!el.name || el.name.length < 3) continue;
            const lowerName = el.name.toLowerCase();
            const nameIdx = lowerText.indexOf(lowerName);
            if (nameIdx !== -1) {
              const startIdx = Math.max(0, nameIdx - 35);
              const endIdx = Math.min(lowerText.length, nameIdx + lowerName.length + 35);
              const surrounding = lowerText.slice(startIdx, endIdx);
              if (/\b(click|select|press|open|follow|choose|tap|button|link)\b/i.test(surrounding)) {
                plannedActions.push({
                  kind: 'action',
                  verb: 'click',
                  ref: el.ref,
                  args: { intent: `Click on "${el.name}"` },
                });
                break;
              }
            }
        ...
        // 2. Check if model expressed scrolling or exploring more content
        if (plannedActions.length === 0) {
          if (/\b(?:scroll|explore more|see more|further down|page down|look below|browse more)\b/i.test(modelText)) {
            plannedActions.push({
              kind: 'action',
              verb: 'scroll',
              args: { direction: 'down', intent: 'Scroll down to view more content' },
            });
          }
        }

        // 3. Check if model expressed that it finished the task or found the result
        if (plannedActions.length === 0) {
          if (/\b(?:task is complete|task completed|task is done|finished|found all|all information|here are the results|here is the information)\b/i.test(modelText)) {
            plannedActions.push({
              kind: 'action',
              verb: 'done',
              args: { result: modelText.slice(0, 300) },
            });
          }
        }

        // 4. Stalled-turn recovery: If model returned text without action, scroll down to discover new elements
        if (plannedActions.length === 0 && emptyPlanStreak >= 1) {
          plannedActions.push({
            kind: 'action',
            verb: 'scroll',
            args: { direction: 'down', intent: 'Auto-scrolling to discover more page elements' },
          });
        }
      }
      ...
      if (plannedActions.length) {
        const d = termDesc(plannedActions[0], brief);
        const intent = plannedActions[0].args?.intent || d;
        await notifyHud('ACTING', `Turn ${iter}: ${intent}`, '#c084fc');
      ...
      if (stuckStreak >= loopCfg.maxStuckRepeats) {
        // Universal Self-Healing Recovery before aborting:
        // 1. If page state is unchanged, scroll down to reveal new elements
        if (!runArtifact._triedStuckScroll) {
          runArtifact._triedStuckScroll = true;
          emit(`  ↳ Self-Healing: page state unchanged; scrolling down to reveal new interactions...`);
          try {
            await exec.execute([{ verb: 'scroll', args: { direction: 'down', amount: 500 } }], session, brief);
          } catch (_) {}
          stuckStreak = 0;
          events.push(`⚠ action repeated with no effect — scrolled down to expose fresh elements`);
          continue;
        }

        // 2. If task mentions a destination URL that hasn't been navigated to, auto-navigate
        const fallbackUrl = resolveDestinationUrl(task, brief.url);
        if (fallbackUrl && fallbackUrl !== brief.url && !runArtifact._triedStuckNav) {
          runArtifact._triedStuckNav = true;
          emit(`  ↳ Self-Healing: navigating directly to target destination: ${cleanUrl(fallbackUrl, LOG_URL_MAX)}`);
          try {
            await session.client.Page.navigate({ url: fallbackUrl });
            await waitUntilLoaded(session.client);
            brief = await session.extract({ inViewportOnly: true });
          } catch (_) {}
          stuckStreak = 0;
        ...
        if (e === 'reflected') { iter--; continue; }
        return await finish(runArtifact, { scratchpad, config: cfg, auditTrail, escalationTracker, visionCallsThisRun, structuralStepsThisRun, visualStepsThisRun, totalPixelsSaved });
      }

      // 4. Validate
      let { ok: validActions, errors: validationErrors } = validate(turnActions, brief.lookup || {}, actions, brief);
      const errors = [
        ...validationErrors,
      ...
      // ── ODVPA: Perception Audit Logging ─────────────────────────────────────
      // Build ScreenGraph for the current turn (DOM/AXTree first principle)
      const currScreenGraph = buildScreenGraph(brief);
      const diffResult = diffScreenGraphs(prevScreenGraph, currScreenGraph);
      prevScreenGraph = currScreenGraph;

      // Count vision calls (take_screenshot executions this turn)
      const visionThisTurn = validActions.some(a => a.verb === 'take_screenshot');
      if (visionThisTurn) {
        visionCallsThisRun++;
        visualStepsThisRun++;
      } else {
        structuralStepsThisRun++;
      }

      // Check screenshot observation for cropMetrics
      const screenshotObs = observations.find((o, i) => validActions[i]?.verb === 'take_screenshot');
      if (screenshotObs?.detail?.cropMetrics) {
        const cm = screenshotObs.detail.cropMetrics;
        if (cm.full_screen_pixels && cm.crop_pixels) {
          totalPixelsSaved += Math.max(0, cm.full_screen_pixels - cm.crop_pixels);
        }
      }

      // Compute privacy risk score from the current page nodes
      const allNodes = [
        ...(brief.elements || []),
        ...(brief.text || []).map(t => ({ ...t, role: t.role || 'heading' })),
      ];
      const privacyRiskScore = calculatePrivacyRisk(allNodes);

      // Determine routing decision from observations (take_screenshot carries vlm meta)
      const vlmMeta = screenshotObs?.vlmMeta || screenshotObs?.raw || {};
      const routeDecisionStr = vlmMeta.routeDecision || (visionThisTurn ? 'VISION_LOCAL' : 'DOM_ONLY');
      const modelTierUsed = vlmMeta.tier
        ? `Local VLM Tier ${vlmMeta.tier} (${vlmMeta.model || 'LFM2.5-VL'})`
        : 'Local SLM (DOM/AXTree resolution)';

      // Log turn to audit trail
      const auditEntry = auditTrail.logTurn({
        turnIndex: iter,
        role: visionThisTurn ? 'vision' : 'reasoning',
        url: brief.url || '',
        nodesCount: currScreenGraph.nodes.length,
        changedNodesCount: diffResult.changedNodes.length,
        piiDetectedCount: allNodes.filter(n => n.sensitive).length,
        piiTypes: allNodes.filter(n => n.piiType).map(n => n.piiType).filter(Boolean),
        privacyRiskScore,
        privacyGateStatus: privacyRiskScore > 0.6 ? 'BLOCKED' : 'SAFE',
        routeDecision: routeDecisionStr,
        modelTierUsed,
        escalationReason: vlmMeta.escalationReason || null,
        offDevicePayloadSent: routeDecisionStr === 'CLOUD',
        bytesSentOffDevice: routeDecisionStr === 'CLOUD' ? JSON.stringify(brief).length : 0,
        action: validActions[0] || null,
      });

      // Track in EscalationTracker for VIR metrics
      const escalationTier = visionThisTurn
        ? (vlmMeta.tier === 'FLORENCE2' ? 'LOCAL_TIER_B'
          : vlmMeta.tier === 'B' ? 'LOCAL_TIER_B'
          : 'LOCAL')
        : 'LOCAL';
      escalationTracker.recordFrame(escalationTier, visionThisTurn ? 'vision' : 'reasoning');
      // ── End ODVPA Audit ─────────────────────────────────────────────────────
      ...
      for (let i = 0; i < observations.length; i++) {
        ...
        if (step.action.verb === 'done') {
          ...
          await notifyHud('READY', 'Task completed successfully.', '#34d399');
          return await finish(runArtifact, { scratchpad, config: cfg, auditTrail, escalationTracker, visionCallsThisRun });
        }
    ...
    await notifyHud('FAILED', `Task error: ${err?.message || err}`, '#ef4444');
...
async function finish(runArtifact, { scratchpad, config = {}, auditTrail, escalationTracker, visionCallsThisRun = 0, structuralStepsThisRun = 0, visualStepsThisRun = 0, totalPixelsSaved = 0 } = {}) {
  // ── ODVPA: Write Audit Trail & Metrics ───────────────────────────────────────
  if (auditTrail) {
    try {
      const auditSummary = auditTrail.getSummary();
      const escalationMetrics = escalationTracker ? escalationTracker.getMetrics() : {};
      const totalTurns = Math.max(1, auditSummary.totalTurns);

      // Vision Invocation Rate (VIR): fraction of turns where vision was called
      const vir = Number(((visionCallsThisRun || 0) / totalTurns).toFixed(4));
      const totalSteps = structuralStepsThisRun + visualStepsThisRun;
      const vlmBypassRate = totalSteps > 0
        ? Number((structuralStepsThisRun / totalSteps).toFixed(4))
        : 1.0;

      // Full ODVPA metrics bundle
      const odvpaMetrics = {
        visionInvocationRate: vir,
        visionCallsTotal: visionCallsThisRun || 0,
        structuralSteps: structuralStepsThisRun,
        visualSteps: visualStepsThisRun,
        vlmBypassRate,
        totalPixelsSaved,
        totalTurns: auditSummary.totalTurns,
        privacyPreservationRate: auditSummary.privacyPreservationRate,
        offDeviceTurns: auditSummary.offDeviceTurns,
        localOnlyTurns: auditSummary.localOnlyTurns,
        piiRedactedTurns: auditSummary.piiRedactedTurns,
        escalation: escalationMetrics,
        reasoning: auditSummary.reasoning,
        vision: auditSummary.vision,
      };

      const auditPayload = {
        kind: 'odvpa-audit-trail',
        version: '1.0',
        runId: runArtifact.id,
        task: runArtifact.task,
        status: runArtifact.status,
        startedAt: runArtifact.startedAt,
        endedAt: new Date().toISOString(),
        metrics: odvpaMetrics,
        entries: auditSummary.entries,
      };

      // Write audit-trail.json to scratchpad directory
      if (scratchpad && scratchpad.dir) {
        const fs = require('fs');
        const path = require('path');
        const auditPath = path.join(scratchpad.dir, 'audit-trail.json');
        try {
          fs.writeFileSync(auditPath, JSON.stringify(auditPayload, null, 2), 'utf8');
          console.error(`\naudit → ${auditPath}`);
        } catch (writeErr) {
          console.error(`[ODVPA] audit trail write failed: ${writeErr.message}`);
        }
      }

      // Emit concise metrics summary to stderr
      const ppr = (auditSummary.privacyPreservationRate * 100).toFixed(1);
      const virPct = (vir * 100).toFixed(1);
      const bypassPct = (vlmBypassRate * 100).toFixed(1);
      console.error(`odvpa · VLM Bypass ${bypassPct}% (${structuralStepsThisRun} structural, ${visualStepsThisRun} visual) · VIR ${virPct}% · privacy-preserved ${ppr}% · PII-redacted-turns ${auditSummary.piiRedactedTurns}`);
    } catch (auditErr) {
      console.error(`[ODVPA] audit metrics error: ${auditErr.message}`);
    }
  }
  // ── End ODVPA Audit ─────────────────────────────────────────────────────────
