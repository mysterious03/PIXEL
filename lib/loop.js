...
async function run({ session, task, config = {} } = {}) {
  ...
  async function notifyHud(status, message, color = null) {
    ...
    try {
      const { generateUpdateHudStatusScript } = require('./live-hud');
      await session.client.Runtime.evaluate({
        expression: generateUpdateHudStatusScript({
          status,
          message,
          color: color || (status === 'THINKING' ? '#f59e0b' : (status === 'ACTING' || status === 'EXECUTING') ? '#c084fc' : status === 'NAVIGATING' ? '#38bdf8' : status === 'FAILED' ? '#ef4444' : '#34d399')
        }),
      }).catch(() => {});
    } catch (_) {}
  }
